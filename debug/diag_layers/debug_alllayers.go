...
func forwardOneLayer(hs []float32, bw *model.Qwen3BlockWeights, config *model.Qwen3Config) []float32 {
	seqLen := 1
...
func main() {
	...
	fmt.Println("Testing EACH layer individually on the raw embedding:")
	for l := 0; l < 28; l++ {
		hs := make([]float32, len(emb))
		copy(hs, emb)
		hs = forwardOneLayer(hs, weights.BlockWeights[l], config)
		...
		fmt.Printf("  Layer %2d: #%d %q (%.2f)\n", l, topIdx, vocab[topIdx], topVal)
	}
}
